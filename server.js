...
const onProxyReq = (proxyReq, req, res, options) => {
  ...
  proxyReq.setHeader('authorization', "bearer_key"=`${process.env.bearer_key}`, "oauth_token="`${process.env.oauth_token}`, "oauth_consumer_secret"=`${process.env.oauth_consumer_secret}`, "oauth_consumer_key"=`${process.env.oauth_consumer_key}`, "oauth_secret"=`${process.env.oauth_secret}`)
